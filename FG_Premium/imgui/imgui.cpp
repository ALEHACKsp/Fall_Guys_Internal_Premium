...
static void ImGui::ErrorCheckBeginEndCompareStacksSize(ImGuiWindow* window, bool write)
{
    ...
    { int n = g.BeginPopupStack.Size;     if (write) *p = (short)n; p++; }// Too few or too many EndMenu()/EndPopup()
    { int n = g.ColorModifiers.Size;      if (write) *p = (short)n; p++; }    // Too few or too many PopStyleColor()
    { int n = g.StyleModifiers.Size;      if (write) *p = (short)n; p++; }    // Too few or too many PopStyleVar()
	{ int n = g.FontStack.Size;           if (write) *p = (short)n; p++; }    // Too few or too many PopFont()
