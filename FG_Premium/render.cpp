...
void update() {
	...
	if (render::menu_is_open) {
		...
		//cursor_manager->static_fields->_Instance_k__BackingField->fields._IsMouseLocked_k__BackingField = false;
	}

	int client_reinput_instance = rand() % 5 - 1;

	if (client_reinput_instance == 1)
		re_input->static_fields->lnEYEzJPkNArQCiSclJonUOvZDE = render::menu_is_open; //Rewired_ReInput__get_inputAllowed
	else if (client_reinput_instance == 2)
		re_input->static_fields->BGxCNmcvACwDGjkKLAqoaDAoFIQM = render::menu_is_open;
	else if (client_reinput_instance == 3)
		re_input->static_fields->HQLaKohzXRajMoKwSNuhmhfTAmCU = render::menu_is_open;
	else if (client_reinput_instance == 4)
		re_input->static_fields->NfbpiUghsbWCBmdQeqEowaCLfKL = render::menu_is_open;
	else
		re_input->static_fields->BUmbOTFTCOeVsllhVMThCInAEem = render::menu_is_open;
	...
	// instant win
	const constexpr auto round_gauntlet_01 = fnv::hash_constexpr("round_gauntlet_01");
	const constexpr auto round_gauntlet_02 = fnv::hash_constexpr("round_gauntlet_02");
	const constexpr auto round_gauntlet_03 = fnv::hash_constexpr("round_gauntlet_03");
	const constexpr auto round_see_saw = fnv::hash_constexpr("round_see_saw");
	const constexpr auto round_dodge_fall = fnv::hash_constexpr("round_dodge_fall");
	const constexpr auto round_chompchomp = fnv::hash_constexpr("round_chompchomp");
	...
	if (fnv::hash_runtime(current_state->klass->_1.name) == StateGameInProgress) {
		...
		auto global_game = reinterpret_cast<FGClient_GlobalGameStateClient_o*>(current_state);
		if (!global_game)
			return;
		...
		client_game_manager->fields._characterDataMonitor->fields._timeToRunNextCharacterControllerDataCheck = FLT_MAX - 1;
		...
		for (auto i = 0; i < player_list->fields.count; i++) {
			...
			if (character->fields._IsLocalPlayer_k__BackingField) {
				...
				if (FGInternal::MOVEMENT::fly_enabled) {
					...
					auto velocity = reinterpret_cast<vector*> (movement + 0x15C);

					if (io.KeysDown[0x57] || io.NavInputs[ImGuiNavInput_LStickUp] > 0.f)
						*velocity = direction * FGInternal::MOVEMENT::fly_speed;
					else
						*velocity = vector(0, 0, 0);

					if (io.KeysDown[VK_SPACE] || io.NavInputs[ImGuiNavInput_Activate] > 0.f) {
						velocity->z = FGInternal::MOVEMENT::fly_speed;
					} else if (io.KeysDown[VK_SHIFT] || io.NavInputs[ImGuiNavInput_Cancel] > 0.f) {
						if (io.KeysDown[0x43] || io.NavInputs[ImGuiNavInput_DpadRight] > 0.f) {
							velocity->z = 0.f;
						} else {
							velocity->z = -FGInternal::MOVEMENT::fly_speed;
						}
				...
				} else if (!FGInternal::MOVEMENT::fly_enabled) {
				...
				if (FGInternal::COLLISIONS::unintentional_move_speed) {
					character->fields._data->fields.unintentionalMoveSpeedThreshold = FGInternal::COLLISIONS::unintentional_move_speed_value;
					character->fields._data->fields.unintentionalMoveSpeedThresholdDuringEmote = FGInternal::COLLISIONS::unintentional_move_speed_emote_value;
				} else if (!FGInternal::COLLISIONS::unintentional_move_speed) {
					character->fields._data->fields.unintentionalMoveSpeedThreshold = VALUES::DEFAULT_VALUES::default_unintentionalMoveSpeedThreshold;
					character->fields._data->fields.unintentionalMoveSpeedThresholdDuringEmote = VALUES::DEFAULT_VALUES::default_unintentionalMoveSpeedThresholdDuringEmote;
				}
				...
				} else if (!FGInternal::MOVEMENT::speed_enabled) {
				...
				} else if (!FGInternal::MOVEMENT::dive_enabled) {
				...
				else if (!FGInternal::MOVEMENT::gravity_enabled) {
					character->fields._data->fields.gravityScale = VALUES::DEFAULT_VALUES::default_gravityScale;
				}

				if (game_level == round_gauntlet_01 || game_level == round_gauntlet_02 || game_level == round_gauntlet_03 
				|| game_level == round_see_saw || game_level == round_dodge_fall || game_level == round_chompchomp 
				|| game_level == round_tip_toe || game_level == round_door_dash) {
					if (!FGInternal::GENERAL::hide_corner_text) {
						...
						if (FGInternal::GENERAL::instant_win) {
							draw_manager::add_text_on_screen({ 5, y }, 0xFF7230DB, 12, "Instant Qualify Available");
						}
					}
					if (FGInternal::GENERAL::instant_win) {
						if (GetAsyncKeyState(0x47)) {
							...
							character->fields._data->fields.gravityScale = -999999.f;
							INPUT space = { 0 };
							...
							character->fields._data->fields.gravityScale = 1.5f;
						}
					}
				}

				if (GetAsyncKeyState(VK_MBUTTON)) {
					if (FGInternal::JUMP::auto_bunny_hop) {
						if (!FGInternal::JUMP::no_jump_delay) {
							FGInternal::JUMP::no_jump_delay = !FGInternal::JUMP::no_jump_delay;
						}
					//
						INPUT space = { 0 };
						space.type = INPUT_KEYBOARD;
				...
				if (FGInternal::JUMP::no_jump_delay) {
					character->fields._data->fields.landingJumpDelay = 0.f;
				} else {
					character->fields._data->fields.landingJumpDelay = VALUES::DEFAULT_VALUES::default_landingJumpDelay;
				}

				if (FGInternal::CARRY::carryTussleChances) {
					character->fields._data->fields.carryAlwaysLoseTussleWhenGrabbed = 0.f;
				} else if (!FGInternal::CARRY::carryTussleChances) {
				...
				if (FGInternal::GRAB::supergrabfeature_enabled) {
					...
					//
					FGInternal::GRAB::grabDetectRadius = false;
					FGInternal::GRAB::grabCheckDistance = false;
					FGInternal::GRAB::grabMaxForce = false;
					FGInternal::GRAB::grabBreakTime = false;
					FGInternal::GRAB::grabCheckPredictionBase = false;
					FGInternal::GRAB::grabImmediateVelocityReduction = false;
					FGInternal::GRAB::grabDragDirectionContribution = false;
					FGInternal::GRAB::grabCooldown = false;
					FGInternal::GRAB::grabRegrabDelay = false;
					FGInternal::GRAB::grabBreakTime = false;
					FGInternal::GRAB::grabReleaseRegrabCooldown = false;
					FGInternal::GRAB::grabBreakAngle = false;
					FGInternal::GRAB::grabSeparationForce = false;
				...
				if (FGInternal::GRAB::grabDetectRadius) {
					if (FGInternal::GRAB::supergrabfeature_enabled)
					{
						FGInternal::GRAB::grabDetectRadius = !FGInternal::GRAB::grabDetectRadius;
					}
				...
				if (FGInternal::GRAB::grabCheckDistance) {
					if (FGInternal::GRAB::supergrabfeature_enabled)
					{
						FGInternal::GRAB::grabCheckDistance = !FGInternal::GRAB::grabCheckDistance;
					}
				...
				if (FGInternal::GRAB::grabMaxForce) {
					if (FGInternal::GRAB::supergrabfeature_enabled)
					{
						FGInternal::GRAB::grabMaxForce = !FGInternal::GRAB::grabMaxForce;
					}
				...
				if (FGInternal::GRAB::grabBreakTime) {
					if (FGInternal::GRAB::supergrabfeature_enabled)
					{
						FGInternal::GRAB::grabBreakTime = !FGInternal::GRAB::grabBreakTime;
					}
				...
				if (FGInternal::GRAB::grabCheckPredictionBase) {
					if (FGInternal::GRAB::supergrabfeature_enabled)
					{
						FGInternal::GRAB::grabCheckPredictionBase = !FGInternal::GRAB::grabCheckPredictionBase;
					}
				...
				if (FGInternal::GRAB::grabImmediateVelocityReduction) {
					if (FGInternal::GRAB::supergrabfeature_enabled)
					{
						FGInternal::GRAB::grabImmediateVelocityReduction = !FGInternal::GRAB::grabImmediateVelocityReduction;
					}
				...
				if (FGInternal::GRAB::grabDragDirectionContribution) {
					if (FGInternal::GRAB::supergrabfeature_enabled)
					{
						FGInternal::GRAB::grabDragDirectionContribution = !FGInternal::GRAB::grabDragDirectionContribution;
					}
				...
				if (FGInternal::GRAB::grabRegrabDelay) {
					if (FGInternal::GRAB::supergrabfeature_enabled)
					{
						FGInternal::GRAB::grabRegrabDelay = !FGInternal::GRAB::grabRegrabDelay;
					}
				...
				if (FGInternal::GRAB::grabCooldown) {
					if (FGInternal::GRAB::supergrabfeature_enabled)
					{
						FGInternal::GRAB::grabCooldown = !FGInternal::GRAB::grabCooldown;
					}
					character->fields._data->fields.grabCooldown = FGInternal::GRAB::grabCooldownBoost;
					character->fields._data->fields.forceReleaseRegrabCooldown = FGInternal::GRAB::grabReleaseRegrabCooldownBoost;
				} else if (!FGInternal::GRAB::grabCooldown) {
					character->fields._data->fields.grabCooldown = VALUES::DEFAULT_VALUES::default_grabCooldown;
				...
				if (FGInternal::GRAB::grabBreakAngle) {
					if (FGInternal::GRAB::supergrabfeature_enabled)
					{
						FGInternal::GRAB::grabBreakAngle = !FGInternal::GRAB::grabBreakAngle;
					}
				...
				if (game_level == round_jinxed) {
					if (FGInternal::ESP::non_jinxed_players_enabled) {
						if (FGInternal::ESP::non_jinxed_enemies_enabled) {
							if (get_character_team_id((uint32_t)player_list->fields.entries->m_Items[i].fields.key) != my_player_team_id) {
								if (!character->fields._ActiveTagAccessory_k__BackingField
									...
									if (get_bounding_box2d(character->fields._collider, vec_min, vec_max))
										draw_manager::add_rect_on_screen(vec_min, vec_max, FGInternal::ESP::jinxed_enemies_color, 0.f, -1, 2.5f);
								}
							}
						} 
						if (FGInternal::ESP::non_jinxed_allies_enabled) {
							if (get_character_team_id((uint32_t)player_list->fields.entries->m_Items[i].fields.key) == my_player_team_id) {
								if (!character->fields._ActiveTagAccessory_k__BackingField
									|| std::uintptr_t(character->fields._ActiveTagAccessory_k__BackingField) != std::uintptr_t(character->fields._infectedAccessory)
									|| !character->fields._ActiveTagAccessory_k__BackingField->fields._isAccessoryEnabled) {
									vector vec_min, vec_max;
									if (get_bounding_box2d(character->fields._collider, vec_min, vec_max))
										draw_manager::add_rect_on_screen(vec_min, vec_max, FGInternal::ESP::jinxed_allies_color, 0.f, -1, 2.5f);
								}
							}
						}
					}
				...
				if (game_level == round_royal_rumble) {
					if (FGInternal::ESP::show_player_with_tail) {
						if (std::uintptr_t(character->fields._ActiveTagAccessory_k__BackingField) == std::uintptr_t(character->fields._tailTagAccessory)) {
							vector vec_min, vec_max;
							if (get_bounding_box2d(character->fields._collider, vec_min, vec_max))
								draw_manager::add_rect_on_screen(vec_min, vec_max, FGInternal::ESP::player_with_tail_color, 0.f, -1, 2.5f);
						}
					}
				}
				if (FGInternal::ESP::enable_players_esp) {
					vector vec_min, vec_max;
					if (get_bounding_box2d(character->fields._collider, vec_min, vec_max))
						draw_manager::add_rect_on_screen(vec_min, vec_max, FGInternal::ESP::players_esp_color, 0.f, -1, 2.5f);
				}
			}
		}
						...
						if (FGInternal::ESP::correct_doors_enabled) {
							if (real_door) {
								...
								if (get_bounding_box2d(real_door->fields._wallCollider, vec_min, vec_max))
									draw_manager::add_rect_on_screen(vec_min, vec_max, FGInternal::ESP::correct_doors_color, 0.f, -1, 2.5f);
							}
						}
						...
						if (FGInternal::ESP::correct_path_enabled) {
							if (tiptoe_platform) {
								...
								if (world_to_screen(world, screen))
									draw_manager::add_filled_rect_on_screen(screen, screen + vector(15, 15), FGInternal::ESP::correct_path_color);
							}
						}
...
void render::load() {
	...
	hwnd = FindWindow(nullptr, "FallGuys_client_game");
